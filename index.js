...
const STAFF_ROLE_IDS = ["1405183233293025382", "1405183143501103236", "1405183318688796803"];
...
const warningSchema = new mongoose.Schema({
  ...
  warnings: [{ reason: String, moderatorId: String, timestamp: Number }],
});
...
app.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (session.metadata.userId && session.metadata.tier) {
      await activatePremium(session.metadata.userId, session.metadata.tier);
    }
  }
...
mongoose.connect(MONGO_URI).then(() => console.log("MongoDB OK")).catch(console.error);
...
async function tryAddXP(userId, channel) {
  ...
  if (u.xp >= need) {
    u.level += 1;
    u.xp = 0;
    if (channel) channel.send("<@" + userId + "> subio a nivel " + u.level);
  }
  await u.save();
}
...
async function registerCommands() {
  try {
    ...
    console.error("Error comandos:", e);
  }
}
...
client.on("interactionCreate", async (i) => {
  if (i.isButton()) {
    if (i.customId === "create_ticket") {
      ...
      return i.reply({ content: "Ticket: " + ch.toString(), ephemeral: true });
    }
  ...
  try {
    ...
    if (cmd === "balance") {
      ...
      const e = new EmbedBuilder().setTitle("Balance de " + i.user.username).addFields({ name: "Efectivo", value: String(u.money), inline: true }, { name: "Banco", value: String(u.bank || 0), inline: true }, { name: "Total", value: String(u.money + (u.bank || 0)), inline: true }).setColor("Green");
    ...
    if (cmd === "8ball") {
      ...
      const e = new EmbedBuilder().setTitle("Bola Magica").addFields({ name: "Pregunta", value: q }, { name: "Respuesta", value: ans[Math.floor(Math.random() * ans.length)] }).setColor("Purple");
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "meme") {
      const memes = ["https://i.imgur.com/2Z8QZ0M.jpg", "https://i.imgur.com/7gFqrNs.jpg", "https://i.imgur.com/xGQ4k2l.jpg"];
    ...
    if (cmd === "userinfo") {
      ...
      const e = new EmbedBuilder().setTitle("Info: " + u.username).setThumbnail(u.displayAvatarURL()).addFields({ name: "ID", value: u.id, inline: true }, { name: "Creado", value: "<t:" + Math.floor(u.createdTimestamp / 1000) + ":R>", inline: true }, { name: "Se unio", value: "<t:" + Math.floor(m.joinedTimestamp / 1000) + ":R>", inline: true }).setColor("Purple");
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "serverinfo") {
      if (!i.guild) return i.reply({ content: "Solo servidores", ephemeral: true });
      const g = i.guild;
      const e = new EmbedBuilder().setTitle(g.name).setThumbnail(g.iconURL()).addFields({ name: "Miembros", value: String(g.memberCount), inline: true }, { name: "Creado", value: "<t:" + Math.floor(g.createdTimestamp / 1000) + ":R>", inline: true }, { name: "Owner", value: "<@" + g.ownerId + ">", inline: true }).setColor("Gold");
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "leaderboard") {
      ...
      if (type === "money") {
        ...
        data = users.map((u) => {
          return { id: u.userId, val: u.money + (u.bank || 0) };
        });
      } else {
        const users = await Level.find({}).sort({ level: -1 }).limit(10);
        data = users.map((u) => {
          return { id: u.userId, val: u.level };
        });
      }
      const desc =
        data
          .map((d, idx) => {
            const valor = type === "money" ? "$" + d.val : "Nivel " + d.val;
            return "**" + (idx + 1) + ".** <@" + d.id + "> - " + valor;
          })
          .join("\n") || "Sin datos";
      const e = new EmbedBuilder().setTitle("Top " + (type === “
    ...
    if (cmd === "premium") {
      ...
      const e = new EmbedBuilder()
        ...
        .addFields({ name: "Beneficios", value: "2x XP en mensajes\n50% mas recompensas\nComandos exclusivos" }, { name: "Planes", value: "Mensual: 3€/mes\nLifetime: 30€ (pago unico)" })
    ...
    if (cmd === "buypremium") {
      ...
      try {
        ...
        const e = new EmbedBuilder().setTitle("Checkout Premium").setDescription("Haz clic abajo para completar el pago\n\nPlan: " + (plan === "monthly" ? "Mensual 3€/mes" : "Lifetime 30€")).setColor("Gold");
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("Ir al Checkout").setStyle(ButtonStyle.Link).setURL(url));
        return i.reply({ embeds: [e], components: [row], ephemeral: true });
      } catch (e) {
        return i.reply({ content: "Error checkout", ephemeral: true });
      }
    }
    ...
    if (cmd === "megaslots") {
      ...
      const e = new EmbedBuilder()
        ...
        .addFields({ name: "Resultado", value: mult > 0 ? "GANASTE x" + mult + "! +" + prize : "Perdiste -" + amt }, { name: "Saldo", value: "**" + (await getBalance(i.user.id)) + "**" })
    ...
    if (cmd === "givepremium") {
      ...
      const e = new EmbedBuilder()
        ...
        .addFields({ name: "Usuario", value: "<@" + target.id + ">", inline: true }, { name: "Plan", value: plan === "monthly" ? "Mensual (30 dias)" : "Lifetime", inline: true }, { name: "Por", value: "<@" + i.user.id + ">", inline: true })
        .setColor("Gold")
        .setTimestamp();
    ...
    if (cmd === "kick") {
      ...
      const e = new EmbedBuilder().setTitle("Usuario Expulsado").addFields({ name: "Usuario", value: target.tag }, { name: "Moderador", value: i.user.tag }, { name: "Razon", value: razon }).setColor("Orange").setTimestamp();
    ...
    if (cmd === "ban") {
      ...
      const e = new EmbedBuilder().setTitle("Usuario Baneado").addFields({ name: "Usuario", value: target.tag }, { name: "Moderador", value: i.user.tag }, { name: "Razon", value: razon }).setColor("Red").setTimestamp();
      await sendLog(i.guild, e);
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "warn") {
      ...
      const e = new EmbedBuilder().setTitle("Usuario Advertido").addFields({ name: "Usuario", value: target.tag }, { name: "Moderador", value: i.user.tag }, { name: "Razon", value: razon }, { name: "Total warns", value: String(count) }).setColor("Yellow").setTimestamp();
      await sendLog(i.guild, e);
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "warnings") {
      ...
      const e = new EmbedBuilder().setTitle("Advertencias de " + target.username).setDescription(desc).setColor("Yellow");
      return i.reply({ embeds: [e], ephemeral: true });
    }
    ...
    if (cmd === "timeout") {
      ...
      const e = new EmbedBuilder().setTitle("Usuario Silenciado").addFields({ name: "Usuario", value: target.tag }, { name: "Moderador", value: i.user.tag }, { name: "Duracion", value: mins + " minutos" }, { name: "Razon", value: razon }).setColor("DarkRed").setTimestamp();
      await sendLog(i.guild, e);
      return i.reply({ embeds: [e] });
    }
    ...
    if (cmd === "setup") {
      ...
      const e = new EmbedBuilder().setTitle("Sistema de Tickets").setDescription("Haz clic en el boton para crear un ticket de soporte.\n\nUn miembro del staff te atendera pronto.").setColor("Blue").setFooter({ text: "Sistema de Soporte" }).setTimestamp();
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("create_ticket").setLabel("Crear Ticket").setStyle(ButtonStyle.Primary));
      try {
    ...
    if (cmd === "announce") {
      ...
      const canal = await i.guild.channels.fetch(canalId).catch(() => null);
      if (!canal || canal.type !== ChannelType.GuildText) return i.reply({ content: "Canal invalido", ephemeral: true });
      const e = new EmbedBuilder().setDescription(mensaje).setColor("Blue").setTimestamp().setFooter({ text: "Anuncio por " + i.user.username, iconURL: i.user.displayAvatarURL() });
